...
function drawSysMenu(){
	...
	menuHeader.textContent = "大家一起改造脑神经";
...
function startLevel() {
	...
	for(i=0; i<level[0].length; i++) {
        point = rotate(0, convertXLocation(level[0][i][0]),convertYLocation(level[0][i][1]));
...
function collisionMagic() {
	...
	if(!B) {
		...
		if (session==0){
			...
			stimidset=session+"";
			numset=levelNum+'';
			timeset=testDuration+'';
			
		}else {
			finalscore=finalscore+";"+levelNum;
			session++;
			type4set=type4set+';'+levelNum;
			stimidset=stimidset+";"+session;
			numset=numset+';'+levelNum;
			timeset=timeset+';'+testDuration;
		}
...
function rotate(angel, oX, oY){
    ...
    centerX = gameW/2;
    centerY = gameH/2;
    //point.x = Math.floor(((oX-centerX)*Math.cos(angel) - (oY-centerY)*Math.sin(angel))+100);
    //point.y = Math.floor(((oY-centerY)*Math.cos(angel) + (oX-centerX)*Math.sin(angel))+100);
	point.x = Math.floor(((oX-centerX)*Math.cos(angel) - (oY-centerY)*Math.sin(angel))/2+centerX);
    point.y = Math.floor(((oY-centerY)*Math.cos(angel) + (oX-centerX)*Math.sin(angel))/2+centerY);
    return point;
}
...
function dp_share() {
    ...
    if (mode == 1) {
        ...
        shareTitle += "【改造脑神经" + mode2LevelNum + "关】我只用了" + ygjscore + ygjscoreunit;
    }
...
function dp_submitScore() {
    if (mode == 1) {
        ...
        if (score < 7)
            ...
            myData.scoreName = "耗时"+score+"＂,伟大的脑神经";
        else if (score < 30)
		...
		if(mode2LevelNum=="A1"){
			timeset=parseInt(ygjscore)+"";
			type4set="1";
			stimidset="1";			
			finalscore=stimidset;
     		numset=(120-parseInt(ygjscore))+'';
     		commentset=	stepNum+'';
     		
			
		}else if(mode2LevelNum=="A2"){
			timeset=timeset+';'+parseInt(ygjscore);
			type4set=type4set+";1";
			stimidset=stimidset+";2";
			finalscore=stimidset;
			numset=numset+";"+(120-parseInt(ygjscore));
     		commentset=	commentset+';'+stepNum;
			alert(commentset);
			post_result();
		}
